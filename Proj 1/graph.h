...
class Graph {
   public:
    ...
    const Graph& operator=(
        const Graph& rhs);  // overloaded assignment operator
   ...
   private:
    ...
    void loadData();  // read data from file and initialize all member
                      // variables
    void insert(int node, int n, int e, int s, int w);
    bool findPath(Node* aNode, int end);  // helper function for recursive
                                          // calls
