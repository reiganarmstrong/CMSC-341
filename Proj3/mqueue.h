...
**
** Function declarations for the mqueue class functions, largely given excpet
** for helper functions made in MQueue
**
**
**
**
...
class Order {
   ...
   private:
    ...
    int m_overWritten;
    // m_material is a value indicating the probability of material
    // availability for valid values refer to enum type Material
    int m_material;  // material availability, for valid values refer to enum
                     // type Material
    int m_quantity;  // quantity of the order, for valid values refer to enum
                     // type Quantity
    // m_arrivalTime is an integer indicating in what order customers placed
    // their order it can be between 1 and 50, at any time the plant only
    // accepts 50 orders at max therefore this value cannot go beyond 50 a
    // value of 1 means this was the first customer who placed the order a
    // value of 50 means this was the 50th customer who placed the order
    int m_arrivalTime;
};
...
class MQueue {
   ...
   private:
     ...
     ******************************************/
    Node* recursiveMerge(MQueue& rhs);
    void recursiveClear(Node* start);
    void preOrderPrint(Node* start) const;
    Node* recursiveCopyNodes(Node* rhsStart);
};
